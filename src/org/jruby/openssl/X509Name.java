...
public class X509Name extends RubyObject {
        ...
        if(!tmp.isNil()) {
            ...
            for(Iterator iter = ((RubyArray)tmp).getList().iterator();iter.hasNext();) {
                ...
                if(entry[2] == null || entry[2].isNil()) {
                    entry[2] = template.callMethod("[]",entry[0]);
                }
                if(entry[2] == null || entry[2].isNil()) {
    ...
    public IRubyObject add_entry(IRubyObject[] args) {
        ...
        IRubyObject type = ((RubyModule)(getRuntime().getModule("OpenSSL").getConstant("X509"))).getClass("Name").getConstant("OBJECT_TYPE_TEMPLATE").callMethod("[]",args[0]);
